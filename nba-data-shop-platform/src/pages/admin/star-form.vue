<template>
  <view class="container">
    <view class="header">
      <view class="title">{{ isEdit ? '编辑球星' : '添加球星' }}</view>
    </view>
    ...
    <view class="form-container">
      <view class="form-section">
        ...
        <view class="form-item">
          <view class="form-label">球星头像</view>
        ...
        <view class="form-item">
          <view class="form-label">球星姓名 *</view>
          <input v-model="formData.name" placeholder="请输入球星姓名" class="form-input" />
        </view>
    ...
    loadPlayerDetail() {
      ...
      getPlayerDetail(this.playerId).then(res => {
        if (res.code === 200) {
          ...
          this.formData.player_type = 'star'
        }
      }).catch(err => {
        console.error('加载球星详情失败', err)
      }).finally(() => {
        uni.hideLoading()
      })
    },
