...
<script>
import { sendSms, login, register } from '@/api/auth'
...
export default {
  ...
  methods: {
    ...
    resetForm() {
      ...
      this.registerForm = { phone: '', smsCode: '' }
      this.isAgree = false
      this.codeText = '获取验证码'
    ...
    getSmsCode() {
      ...
      sendSms(form.phone).then(res => {
        if (res.code === 200) {
          ...
          uni.showToast({ 
            title: '验证码已发送，开发环境验证码为: ' + res.data.code, 
            icon: 'none',
            duration: 3000
          })
        }
    ...
    login() {
      ...
      // 调用后端API登录
      login(this.loginForm.phone, this.loginForm.smsCode).then(res => {
        if (res.code === 200) {
          // 保存token和用户信息
    ...
    register() {
      ...
      // 调用后端API注册
      register(this.registerForm.phone, this.registerForm.smsCode).then(res => {
        if (res.code === 200) {
          // 保存token和用户信息
          uni.setStorageSync('token', res.data.token)
          uni.setStorageSync('userInfo', res.data.user)
